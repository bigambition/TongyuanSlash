...
function SmartAI:aoeIsEffective(card, to, source)
	...
	if source:hasSkills("noswuyan|zhongyong") or to:hasSkills("noswuyan|luakaoshen|zhongyong|laoshi") then
		return false
	end
...
function SmartAI:hasTrickEffective(card, to, from)
	...
	if to:objectName() ~= from:objectName() then
		if from:hasSkills("noswuyan|zhongyong") or to:hasSkills("noswuyan|luakaoshen|zhongyong|laoshi") then
	...
	if from:hasSkill("luaxiyu") and to:isFemale() and card:isKindOf("Duel")	then return false end
...
function SmartAI:useTrickCard(card, use)
	...
	if card:isKindOf("AOE") then
		...
		if self.player:hasSkills("noswuyan|zhongyong")
...
function SmartAI:doNotDiscard(to, flags, conservative, n, cant_choose)
	...
	if #enemies == 1 and self:hasSkills("noswuyan|qianxun|weimu|luakaoshen|zhongyong|laoshi", enemies[1]) and self.room:alivePlayerCount() == 2 then conservative = false end
