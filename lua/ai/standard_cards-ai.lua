...
function sgs.getDefenseSlash(player)
	...
	local huzhuJink=0
	if player:hasLordSkill("luahuzhu") then
		local lieges = global_room:getLieges("yuan", player)
		for _, liege in sgs.qlist(lieges) do
			if sgs.compareRoleEvaluation(liege,"rebel","loyalist") == sgs.compareRoleEvaluation(player,"rebel","loyalist") then
				huzhuJink = huzhuJink + getCardsNum("Jink",liege)
				if liege:hasArmorEffect("EightDiagram") then huzhuJink = huzhuJink + 0.8 end
			end
		end
		defense = defense + huzhuJink
	end
	...
	if defense > 0 and attacker:objectName() ~= player:objectName() then
		...
		if need_double_jink and getKnownCard(player, "Jink", true, "he") < 2
			...
			and (not player:hasLordSkill("luahuzhu") or huzhuJink < 2) then
			defense = 0
		end
	...
	if player:getHandcardNum() == 0 and hujiaJink == 0 and huzhuJink == 0 and not player:hasSkill("kongcheng") then
...
function SmartAI:slashProhibit(card, enemy, from)
	...
	if card and from:hasSkill("luaxiyu") and enemy:isFemale() then return true end
...
sgs.ai_skill_invoke.EightDiagram = function(self, data)
	...
	if self:hasSkills("tiandu|leiji|gushou") then
		...
		if sgs.huzhusource and not self:isFriend(sgs.huzhusource) and (sgs.huzhusource:hasFlag("dahe") or self.player:hasFlag("dahe")) then return true end
	...
	if sgs.huzhusource and (not self:isFriend(sgs.huzhusource) or sgs.huzhusource:hasFlag("dahe")) then return false end
	...
	local tohelp = self.room:getTag("tohelp"):toPlayer()
	if tohelp and tohelp:isAlive() then
		return self:isFriend(tohelp)
	end
	return true
end
...
function sgs.ai_armor_value.EightDiagram(player, self)
	...
	if self.role == "loyalist" and self.player:getKingdom() == "yuan" and not self:hasSkills("bazhen|yizhong") and self.room:getLord() and self.room:getLord():hasLordSkill("luahuzhu") then
		return 5
	end
	return 4
end
...
function AG_isEffective(card, to, from)
	...
	if to:hasSkill("chidao") and to:getMark("@chidao") > 0 then return false end
...
function SmartAI:useCardAmazingGrace(card, use)
	...
	if self:needKongcheng() and self.player:getHandcardNum() == 1 or self.player:hasSkills("nosjizhi|jizhi|weikou") then
		suf = 0.6
		coeff = 0.6
	end
	for _, player in sgs.qlist(self.room:getOtherPlayers(self.player)) do
...
function SmartAI:getDangerousCard(who)
	...
	if lord and lord:hasLordSkill("luahuzhu") and self:isEnemy(lord) and armor and armor:isKindOf("EightDiagram") and who:getKingdom() == "yuan" then
		return armor:getEffectiveId()
	end
...
---------------
	...
	if eightdiagram then
		...
		if self.role == "loyalist" and self.player:getKingdom()=="yuan" and not self.player:hasSkill("bazhen") and
			lord and lord:hasLordSkill("luahuzhu") and (lord:objectName() ~= NextPlayer:objectName() and NextPlayerisEnemy or lord:getArmor()) then
			return eightdiagram
		end
	end
